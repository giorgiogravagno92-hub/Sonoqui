...
const WP_DATA_FILE = path.join(__dirname, '../../wp-simulator-data.json');
...
const loadWPData = (): WPData => {
  try {
    if (fs.existsSync(WP_DATA_FILE)) {
      const raw = fs.readFileSync(WP_DATA_FILE, 'utf-8');
...
const saveWPData = (data: WPData) => {
  try {
    fs.writeFileSync(WP_DATA_FILE, JSON.stringify(data, null, 2), 'utf-8');
  } catch (err) {
